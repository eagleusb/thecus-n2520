...
class ext_File {
	function chmod( $item, $mode ) {
		if( ext_isFTPMode() ) {
			...
			return @chmod( cutf8_decode($item), $mode );
		}
	}
	...
	function chmodRecursive( $item, $mode ) {
		if( ext_isFTPMode() ) {
			...
			return chmod_recursive( cutf8_decode($item), $mode );
		}
	}
	function copy( $from, $to ) {
		if( ext_isFTPMode() ) {
			...
			return copy( cutf8_decode($from), cutf8_decode($to) );
		}
	}
	...
	function mkdir( $dir, $perms ) {
		if( ext_isFTPMode() ) {
			...
			return mkdir( cutf8_decode($dir), $perms || 0022);
		}
	}
	function mkfile( $file ) {
		if( ext_isFTPMode() ) {
			...
			return @touch( cutf8_decode($file) );
		}
	}
	function unlink( $item ) {
		if( ext_isFTPMode() ) {
			...
			return unlink( cutf8_decode($item) );
		}
	}
	...
	function remove( $item ) {
		if( ext_isFTPMode() ) {
			...
			return remove( cutf8_decode($item) );
		}
	}
	function rename( $oldname, $newname ) {
		if( ext_isFTPMode() ) {
			...
			return rename( cutf8_decode($oldname), cutf8_decode($newname) );
		}
	}
	function opendir( $dir ) {
		if( ext_isFTPMode() ) {
			...
			return opendir( cutf8_decode($dir) );
		}
	}
	...
	function file_exists( $file ) {
		if( ext_isFTPMode() ) {
			...
			if( !is_array( $file )) {
				$dir = dirname( $file );
				$file = array( 'name' => gbasename( $file ) );
			}
			...
			return file_exists( cutf8_decode($file) );
		}
	}
	function filesize( $file ) {
		if( ext_isFTPMode() ) {
			...
			return shell_exec("ls -al \"$file\" | awk '{print $5}'");
		}
	}
	function fileperms( $file ) {
		if( ext_isFTPMode() ) {
			if( isset( $file['rights'])) {
				...
				$info = get_item_info( dirname( $file), gbasename( $file ) );
				$perms = $info['rights'];
			}
	...
	function file_get_contents( $file ) {
		if( ext_isFTPMode() ) {
			...
			return file_get_contents( cutf8_decode($file) );
		}
	}
	function file_put_contents( $file, $data ) {
		if( ext_isFTPMode() ) {
			...
			return file_put_contents( cutf8_decode($file), $data );
		}
	}
	...
	function is_writable( $file ) {
		...
		if( ext_isFTPMode() ) {
			...
			if( !is_array( $file )) {
				$file = get_item_info(dirname($file), gbasename($file));
			}
			...
			//Disable for ACL support
			//return is_writable( $file );
			$iswrite=getWritePermission($_SESSION['s_user'],$_SESSION['s_gid'],$file);
			//printf("file=%s iswrite=%s \n",$file,$iswrite);
			return $iswrite;
	...
	function is_readable( $file ) {
		if( ext_isFTPMode() ) {
			...
			$isread=getReadPermission($_SESSION['s_user'],$_SESSION['s_gid'],$file);
			//printf("file=%s isread=%s \n",$file,$isread);
			return $isread;
		}
	}
	/**
	...
	function is_deletable( $file ) {
		...
		if(!$isWindows && @$GLOBALS['ext_File']->geteuid() != @$GLOBALS['ext_File']->fileowner($file)) {
			$isdeletable=getWritePermission($_SESSION['s_user'],$_SESSION['s_gid'],$file);
			return $isdeletable;
			//return (substr(decoct(@fileperms($file)),-3) == '777' || $isdeletable );
		}
		if($isWindows && $GLOBALS['ext_File']->geteuid() != $GLOBALS['ext_File']->fileowner($file)) {
		...
		//$isdeletable=@is_writable(dirname($file));
		$isdeletable=getWritePermission($_SESSION['s_user'],$_SESSION['s_gid'],$file);
		//printf("file=%s is_deletable=%s \n",$file,$isdeletable);
		return $isdeletable;
	}
