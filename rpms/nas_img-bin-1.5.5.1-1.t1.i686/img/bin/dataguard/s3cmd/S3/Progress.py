...
class Progress(object):
    ...
    def __init__(self, labels, total_size, file):
        self._stdout = sys.stdout
        self.new_file(labels, total_size)
        self._progress_file = file
        self.progress_file = open(file, 'w+')
    ...
    def output_labels(self):
        self._stdout.write(self.LF_char)
...
class ProgressCR(Progress):
    ...
    def display(self, new_file = False, done_message = None):
        ...
        if (sec_elapsed > 0):
            ...
            print_speed = (0, "")
        #self._stdout.write(self.LF_char)
        output = " %(total)s bytes  %(percent)3d%% in %(elapsed)4ds  %(speed)7.2f %(speed_coeff)sB/s" % {
            "total" : self.total_size,
            "percent" : self.total_size and (self.current_position * 100 / self.total_size) or 0,
            "elapsed" : sec_elapsed,
            "speed" : print_speed[0],
        ...
        self.progress_file.write(output)
        self.progress_file.write(self.LF_char)
        self.progress_file.flush()
        #self.progress_file.write(self.CR_char)
