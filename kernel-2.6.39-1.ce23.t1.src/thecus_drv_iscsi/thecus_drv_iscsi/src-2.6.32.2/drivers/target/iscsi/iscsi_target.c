	...
	struct iscsi_conn *conn)
{
	...
	case ISCSI_LOGOUT_REASON_CLOSE_SESSION:
		printk(KERN_INFO "iSCSI session logout successful, setting"
...
static int iscsit_build_sendtargets_response(struct iscsi_cmd *cmd)
{
	...
	printk("login_ip: %s, local_ip:%s , buffer_len=%d\n", conn->login_ip, conn->local_ip, buffer_len);
	spin_lock(&tiqn_lock);
	list_for_each_entry(tiqn, &g_tiqn_list, tiqn_list) {
		...
		list_for_each_entry(tpg, &tiqn->tiqn_tpg_list, tpg_list) {
			...
			list_for_each_entry(tpg_np, &tpg->tpg_gnp_list,
				...
				if(memcmp(conn->local_ip,tpg_np->tpg_np->np_ip,strlen(tpg_np->tpg_np->np_ip)) == 0) {
			...
			list_for_each_entry(tpg_np, &tpg->tpg_gnp_list,
						tpg_np_list) {
				if(memcmp(conn->local_ip,tpg_np->tpg_np->np_ip,strlen(tpg_np->tpg_np->np_ip)) == 0) continue;
				else {
...
int iscsit_release_sessions_for_tpg(struct iscsi_portal_group *tpg, int force)
{
	...
	printk(KERN_INFO "Released %d iSCSI Session(s) from Target Portal"
