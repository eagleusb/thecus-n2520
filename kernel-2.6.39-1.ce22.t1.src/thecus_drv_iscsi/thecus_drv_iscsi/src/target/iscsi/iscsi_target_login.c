...
static void iscsi_handle_login_thread_timeout(unsigned long data)
{
	...
	printk(KERN_INFO "iSCSI Login timeout on Network Portal %s:%hu\n",
			np->np_ip, np->np_port);
...
static int __iscsi_target_login_thread(struct iscsi_np *np)
{
	...
	*/
	...
	if (np->np_thread_state != ISCSI_NP_THREAD_ACTIVE) {
		spin_unlock_bh(&np->np_thread_lock);
		printk(KERN_INFO "iSCSI Network Portal on %s:%hu currently not"
			" active.\n", np->np_ip, np->np_port);
		iscsit_tx_login_rsp(conn, ISCSI_STATUS_CLS_TARGET_ERR,
	...
	snprintf(conn->local_ip, sizeof(conn->local_ip), "%s", np->np_ip);
...
new_sess_out:
	if ((conn->login_ip) && (conn->sess) && (conn->sess->sess_ops) && (conn->sess->sess_ops->TargetName) && (conn->sess->sess_ops->InitiatorName)) {
		printk(KERN_INFO "iSCSI Login negotiation failed. |%s|%s|%s\n",conn->login_ip, conn->sess->sess_ops->TargetName,conn->sess->sess_ops->InitiatorName);
        } else {
		printk(KERN_INFO "iSCSI Login negotiation failed. |||\n");
	}
