...
if( $action == 'include_javascript' ) {
  	...
  	include( _EXT_PATH.'/scripts/'.gbasename(extGetParam($_REQUEST, 'file' )).'.php');
  	ext_exit();
}
...
if( file_exists( _EXT_PATH . '/include/'. strtolower(gbasename( $action )) .'.php') ) {
	require_once( _EXT_PATH . '/include/'. strtolower(gbasename( $action )) .'.php');
}
