...
/*************************
  ...
  ********************************************
  ...
  $Source: /cvsroot/coppermine/stable/include/exif.php,v $
  $Revision: 1.4 $
  $Author: gaugau $
...
//echo "type1 = ${type} ++ tag = ${tag} ++ intel = ${intel} ++ data = ${data}<br>";
  if($type=="ASCII") {
    //do nothing
    ...
    $data = bin2hex($data);
//if($tag=="010f" || $tag=="1001") echo "type2 = ${type} ++ tag = ${tag} ++ intel = ${intel} ++ data = ${data}<br>";
    if($intel==1) {
      if($tag=="a403"){
        $data=substr($data,0,4);
      }else if($tag=="9207" || $tag=="9209" || $tag=="8827" | $tag=="a002" || $tag=="a003"){
...
//if($tag=="1001") echo "data1 = ".${data}."<br>";
    if($intel==0 && ($type=="USHORT" || $type=="SSHORT")) $data = substr($data,0,4);
    $data=hexdec($data);
//if($tag=="1001") echo "type = ${type} ++ tag = ${tag} ++ intel = ${intel} ++ data = ${data}<br>";
...
//echo "tag = ".$tag." ++ tag name = ".$tag_name."<br>";
//if($tag=="1001")  echo "tag = ".$tag." ++ tag name = ".$tag_name."<br>";
...
//echo "tag_name = ${tag_name}<br>";
    ...
    $formated_data = formatData($type,$tag,$intel,$data);
//if($tag=="1001") echo "format = ".$formated_data."<br>";
