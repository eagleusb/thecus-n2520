...
function openActionDialog( caller, action ) {
	...
	switch( action ) {
      ...
      dialog.addKeyListener(27, dialog.hide, dialog);
			...
			//dialog_status.getEl().addClass(['ext_statusbar', 'donebar']);
		...
		case 'extract':
			...
			break;
		case 'download':
			document.location = '<?php echo gbasename($GLOBALS['script_name']) ?>?option=com_extplorer&action=download&item='+ encodeURIComponent(ext_itemgrid.getSelectionModel().getSelected().get('name')) + '&dir=' + encodeURIComponent( datastore.directory );
			break;
	}
}
		...
		callback: function(options, success, response ) {
			if( success ) {
				...
				if( json.success ) {
					...
					} catch(e) { dirTree.getRootNode().reload(); datastore.reload();}
				} else {
...
function statusBarMessage( msg, isLoading, success ) {
	...
	if( isLoading ) {
		statusBar.removeClass('donebar');
		try { dialog_status.getEl().removeClass('donebar') } catch(e){};
	}
	else {
		try { dialog_status.getEl().addClass('donebar'); } catch(e){}
		statusBar.addClass('donebar');
	}
	if( success ) {
